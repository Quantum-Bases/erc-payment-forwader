...
async function forwardPayments(req, res) {
  try {
    ...
    if (!utils.isValidEthereumAddress(user_address))
      return res.status(401).send("invalid recipient address");
    ...
    if (!transaction)
      return res.status(401).send("Error in creating trnasection");
